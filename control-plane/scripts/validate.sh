#!/usr/bin/env bash
# Runs deterministic control-plane checks and keeps Docker validation opt-in.
set -euo pipefail
...
log "validate: json"
require_cmd jq
run jq . "${ROOT}/src/features/game-catalog/maps.json" >/dev/null
run jq . "${ROOT}/package.json" >/dev/null
run jq . "${ROOT}/package-lock.json" >/dev/null

log "validate: yaml"
require_cmd ruby
run ruby -ryaml -e "YAML.safe_load(File.read('${REPOSITORY_ROOT}/deploy/compose/control-plane.compose.yaml'), aliases: false, filename: '${REPOSITORY_ROOT}/deploy/compose/control-plane.compose.yaml')" >/dev/null
