...
function playerMatchesQuery(player: PlayerRow, query: string): boolean {
  return (
    !query ||
    [player.userid, player.name, player.steam_id64 ?? ''].some((value) =>
...
function createPlayerRow(player: PlayerRow): HTMLElement {
  ...
  meta.textContent = player.steam_id64 ? player.steam_id64 : 'SteamID64 not observed by RCON';
