...
async function launchFavorite(context: FavoriteActionContext, workshopId: string): Promise<void> {
  ...
  void sendPostRequest('/api/workshop-map', {
...
async function deleteFavorite(
  ...
  fetchJson<{ message: string }>(`/api/workshop-favorites/${context.serverId}/${favoriteId}`, {
    method: 'DELETE',
  })
    .then((data) => {
      showToast(data.message, 'success');
...
export async function handleFavoriteAction(
  ...
  if (button.dataset.favoriteAction === 'delete')
    await deleteFavorite(context, favoriteId, workshopId);
}
