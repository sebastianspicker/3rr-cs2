...
export function initPracticeControls(serverId: string): void {
  ...
  bindPreset(serverId, {
    values: [0, 1, 2],
    endpoint: 'infinite-ammo-toggle',
    container: '#inf-ammo-presets',
    idPrefix: 'inf_ammo_',
    label: 'Infinite ammo',
  });
  bindPreset(serverId, {
    values: [0, 5, 10, 15, 20],
    endpoint: 'set-freezetime',
    container: '#freezetime-presets',
    idPrefix: 'freezetime_',
    label: 'Freeze time',
  });
  bindPreset(serverId, {
    values: [0, 800, 1600, 3200, 16000],
    endpoint: 'set-startmoney',
    container: '#startmoney-presets',
    idPrefix: 'startmoney_',
    label: 'Start money',
  });
  bindPreset(serverId, {
    values: [0, 1, 2, 3],
    endpoint: 'bot-difficulty',
    container: '#bot-difficulty-presets',
    idPrefix: 'bot_difficulty_',
    label: 'Bot difficulty',
  });
  bindPreset(serverId, {
    values: [1, 2, 5, 60],
    endpoint: 'set-roundtime',
    container: '#roundtime-presets',
    idPrefix: 'roundtime_',
    label: 'Round time',
  });
  bindPreset(serverId, {
    values: [10, 15, 30, 45, 90],
    endpoint: 'set-buytime',
    container: '#buytime-presets',
    idPrefix: 'buytime_',
    label: 'Buy time',
  });
...
export function initScrimControls(serverId: string): void {
  ...
  bindPreset(serverId, {
    values: [16, 24, 30],
    endpoint: 'set-maxrounds',
    container: '#maxrounds-presets',
    idPrefix: 'maxrounds_',
    label: 'Max rounds',
  });
  ...
  on('#overtime_on', 'click', () => {
    void sendPostRequest('/api/set-overtime', {
      server_id: serverId,
      enable: 1,
      ot_rounds: currentOtMaxrounds,
    })
      .then((d) => {
        showToast(d.message, 'success');
        setToggleActive('overtime', 1);
      })
    ...
    [
      {
        id: 'pause_game',
        endpoint: '/api/pause',
        prompt: 'Pause the game?',
        fallback: 'Pause failed.',
      },
      {
        id: 'unpause_game',
        endpoint: '/api/unpause',
        prompt: 'Unpause the game?',
        fallback: 'Unpause failed.',
      },
      {
        id: 'restart_game',
      ...
      {
        id: 'start_warmup',
        endpoint: '/api/start-warmup',
        prompt: 'Start warmup?',
        fallback: 'Warmup failed.',
      },
      {
        id: 'knife_start',
        endpoint: '/api/start-knife',
        prompt: 'Start knife round?',
        fallback: 'Knife start failed.',
      },
      {
        id: 'swap_team',
        endpoint: '/api/swap-team',
        prompt: 'Swap teams?',
        fallback: 'Swap teams failed.',
      },
      {
        id: 'matchzy_match',
        endpoint: '/api/matchzy-match',
        prompt: 'Start match via MatchZy?',
        fallback: 'Start match failed.',
      },
      {
        id: 'matchzy_abort',
        endpoint: '/api/matchzy-abort',
        prompt: 'Abort the current match?',
