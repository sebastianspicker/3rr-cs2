...
function gameSetupElements(): GameSetupElements | null {
  ...
  if (!typeContainer || !modeContainer || !mapSelect || !gameTypeValue || !gameModeValue)
    return null;
...
function bindModeSelectors(elements: GameSetupElements): void {
  elements.typeContainer.addEventListener('click', (event) => {
    const type = (event.target as HTMLElement).closest<HTMLButtonElement>('[data-game-type]')
      ?.dataset.gameType;
    if (!type) return;
  ...
  elements.modeContainer.addEventListener('click', (event) => {
    const mode = (event.target as HTMLElement).closest<HTMLButtonElement>('[data-game-mode]')
      ?.dataset.gameMode;
    if (!mode) return;
...
function loadInitialMode(elements: GameSetupElements): void {
  const buttons = [
    ...elements.typeContainer.querySelectorAll<HTMLButtonElement>('[data-game-type]'),
  ];
  const initial =
    buttons.find((button) => button.dataset.gameType === elements.gameTypeValue.value) ??
    buttons.at(0);
