...
function createSessionStore(nodeEnv: string, redisClient: RedisClient) {
  if (redisClient) {
...
export function configureSecurity(
  ...
  app.use(
    express.static(path.join(packageRoot, 'web', 'generated'), {
      maxAge: 0,
      immutable: false,
    })
  );
  app.use(
    session({
      ...
      rolling: true,
      store: createSessionStore(nodeEnv, redisClient),
