...
Start the control plane and sign in. Then remove `DEFAULT_USERNAME` and
...
`../scripts/verify.sh` runs the control-plane checks with the host-updater and
server-bootstrap checks.

## Backup and recovery

Stop the control plane before taking a file-level SQLite backup. Protect the database
...
Restore a database only with a compatible application version. Start the control plane,
