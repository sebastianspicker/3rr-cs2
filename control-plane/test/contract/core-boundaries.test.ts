...
import { RconSocketRegistry } from '../../src/integrations/rcon/rconSocketRegistry';
import {
  isRconCommandAllowed,
  sanitizeBackupFileName,
  sanitizeCfgName,
} from '../../src/integrations/rcon/rconCommandPolicy';
import { parseServerId } from '../../src/features/server-access/parseServerId';
import { mapsConfig } from '../../src/features/game-catalog/mapsConfig';
...
test('map configuration only executes safe, provisioned cfg targets', () => {
  const cfgRoot = resolve(process.cwd(), '..', 'server-bootstrap', 'assets', 'cfg');
