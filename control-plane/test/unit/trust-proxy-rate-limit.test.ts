...
import { configureRateLimits } from '../../src/app/rateLimits';
import { configureSecurity } from '../../src/app/security';
import { rateLimitClientKey } from '../../src/shared/clientAddress';
...
async function withRateLimitProbe(
  ...
  const app = express();
  configureSecurity(app, 'test', process.cwd(), null);
  configureRateLimits(app, 'production', null);
...
test('production security uses an explicitly injected Redis session store', () => {
  const originalSessionSecret = process.env.SESSION_SECRET;
  process.env.SESSION_SECRET = 'strong-session-secret-with-symbols-123456!';
  const redisClient = {
    get: async () => null,
    set: async () => 'OK',
    del: async () => 1,
  ...
  try {
    const app = express();
    assert.doesNotThrow(() => configureSecurity(app, 'production', process.cwd(), redisClient));
  } finally {
