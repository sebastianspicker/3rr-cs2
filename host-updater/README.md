...
The repository source now lives in `host-updater`, but the systemd unit keeps
the established `/opt/3rr/apps/maintain/updater/` installation path for
existing hosts. Copy this directory's script, libraries, and configuration to
that path as shown above. Before enabling the timer, run:
...
- this module can be used without the control plane
