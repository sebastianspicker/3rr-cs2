...
test('workshop favorite update distinguishes duplicate conflicts from persistence failures', async () => {
  ...
  try {
    await withAuthedServer(async (baseUrl, auth) => {
      const base = `${baseUrl}/api/workshop-favorites/${serverId}`;

      const first = await postAuthedJson(baseUrl, auth, `/api/workshop-favorites/${serverId}`, {
        workshop_id: '32345678901',
        name: 'First Favorite',
      });
      assert.equal(first.status, 201);
      const firstBody = (await first.json()) as { favorite: { id: number } };

      const second = await postAuthedJson(baseUrl, auth, `/api/workshop-favorites/${serverId}`, {
        workshop_id: '42345678901',
